...
const SwitchNode = ({ id, data, isConnectable }) => {
  ...
  const portElements = useMemo(() => {
    return Array.from({ length: portsCount }, (_, index) => (
      <div
        key={index}
        className={`${data.connected ? "bg-green-500" : "bg-red-600"} p-[3px] rounded-full`}
      />
    ));
  ...
  return (
    <div
      className={`flex ${data.isVisible ? "visible" : "hidden"} justify-center rounded bg-gradient-to-b from-green-400 to-teal-800 text-black w-[220px]`}
    >
